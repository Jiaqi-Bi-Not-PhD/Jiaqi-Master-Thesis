...
\begin{align} 
    ...
    &+\sum_{j=1}^J \log S_{j_p}(a_{j_p}|\mathbf{x}_{j_p}, z_j)
\end{align}
...
\begin{align} 
    ...
    &+\sum_{j=1}^J \int_{\mathbf{x}_{mis}, z_j} \log S_{j_p}(a_{j_p}|\mathbf{x}_{j_p}, z_j)\\
    &\times f(x_{ij,mis}, z_j|W_{ij}, \boldsymbol{\theta}^{(r)})dx_{ij,mis}dz_j
\end{align}
...
\begin{align} 
    ...
    &+\sum_{j=1}^J\frac{1}{M_j}\sum_{m=1}^{M_j}\log S_{j_p}(a_{j_p}|x_{j_p}^{(m)}, z_j^{(m)})\\
