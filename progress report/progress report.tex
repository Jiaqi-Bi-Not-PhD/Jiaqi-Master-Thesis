...
Thus, we propose to sample the PRS using the information that we have already known. Denote $\mathbf{X}_{j,1}$ as the PRS scores vector in family $j$, and $\mathbf{X}_{j,2}$ the mutation status vector in family $j$. We then define $\mathbf{W}=(\log(\mathbf{t}_j)\odot \boldsymbol{\delta}_j, p_j, \mathbf{c}_j, \mathbf{X}_{j,2})$.
