...
\begin{align}
    H(t_{ij}|\mathbf{x}_{ij}, z_j)&=\int_0^{t}h_{ij}(u|x_{ij}, g_{ij}, z_j)du\\
...
It is crucial to address the ascertainment bias. 
...
\end{equation}
where $\tilde{\ell}$ is the log-likelihood with ascertainment correction, and $\ell$ is the crude log-likelihood. Define $\mathbf{x}_{p_j}$ the covariate matrix for proband in family $j$.
