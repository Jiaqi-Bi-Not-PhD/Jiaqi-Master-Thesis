...
\begin{align} 
    ...
    &- \sum_{j=1}^{n_j} (1- z_j H_{j_p}(a_{j_p}|\mathbf{x}_{j_p}))
\end{align}
...
\begin{align} 
    ...
    &\times f(x_{ij,mis}, z_j|W_{ij}, \boldsymbol{\theta}^{(r)})dx_{ij,mis}dz_j\\
    &-\sum_{j=1}^J \int_{\mathbf{x}_{mis}, z_j} \log (1-S_{j_p}(a_{j_p}|\mathbf{x}_{j_p}, z_j))\\
    &\times f(x_{ij,mis}, z_j|W_{ij}, \boldsymbol{\theta}^{(r)})dx_{ij,mis}dz_j
\end{align}
such that we need to integrate out the joint density of the frailty term and the missing covariate from $f(x_{ij,mis}, z_j|W_{ij}, \boldsymbol{\theta}^{(r)})$. 
The $W_{ij}=(t_{ij}, \delta_{ij}, p_{ij}, c_{j}, x_{ij,obs})$ is the observed data including proband information and event information. 
...
\begin{align} 
    f(z_j,x_{mis,ij}|W_{ij}, \boldsymbol{\theta}^{(r)})=f(x_{mis, ij}|W_{ij}, z_j, \boldsymbol{\theta}^{(r)})f(z_j|W_{ij}, \boldsymbol{\theta}^{(r)})
\end{align}
while this has a form of efficient sampling, so we can write
\begin{align} 
    f(z_j, x_{mis,ij}|W_{ij}, \boldsymbol{\theta}^{(r)})&\propto f(t_{ij}, \delta_{ij}|x_{mis,ij}, x_{obs,ij}, z_j, a_{j_p}, \boldsymbol{\beta}^{(r)})\\
    &\times f(x_{mis,ij}|W_{ij}, \psi^{(r)})f(z_j|\upsilon^{(r)})
\end{align}
Clearly, we know $f(t_{ij}, \delta_{ij}|x_{mis,ij}, x_{obs,ij}, z_j, \boldsymbol{\beta}^{(r)})$ is the likelihood of one single observation $j$ in family $j$, also we know the distribution of $f(x_{ij}|\psi)$, as well as the frailty distribution $f(z_j|\upsilon)$. 
Therefore, in our case, we can write 
\begin{align} 
    f(z_j, x_{mis,ij}|W_{ij}, \boldsymbol{\theta}^{(r)})&\propto f(z_j|\upsilon^{(r)})\Big [ \prod_{i=1}^{n_j} f(x_{mis,ij}|W_{ij}, \psi^{(r)})\\ 
    &\times h^{(r)}(t_{ij}|x_{ij}, z_j)^{\delta_{ij}}\exp (-H^{(r)}(t_{ij}|x_{ij}, z_j))\Big ]
\end{align}
In general, without the specification of the frailty distribution, the E-step in MCEM can be written as 
\begin{align} 
    Q(\boldsymbol{\theta}|\boldsymbol{\theta}^{(r)})&=\sum_{j=1}^J \frac{1}{M_j}\sum_{m=1}^{M_j}\sum_{i=1}^{n_j} \Big ( \delta_{ij}\log h(t_{ij}|x_{ij}^{(m)}, z_j^{(m)}) - H(t_{ij}|x_{ij}^{(m)}, z_j^{(m)})\Big )\\
    &+\sum_{j=1}^J\frac{1}{M_j}\sum_{m=1}^{M_j}\log (1-S_{j_p}(a_{j_p}|x_{j_p}^{(m)}, z_j^{(m)}))\\
    &+\sum_{j=1}^J\frac{1}{M_j}\sum_{m=1}^{M_j}\sum_{i=1}^{n_j}\log f(x_{ij,mis}^{(m)}, z_j^{(m)}|W_{ij}, \boldsymbol{\theta})\\
    &=\sum_{j=1}^J \frac{1}{M_j}\sum_{m=1}^{M_j}\sum_{i=1}^{n_j} \Big ( \delta_{ij}\log h(t_{ij}|x_{ij}^{(m)}, z_j^{(m)}) - H(t_{ij}|x_{ij}^{(m)}, z_j^{(m)})\Big )\\
    &+\sum_{j=1}^J\frac{1}{M_j}\sum_{m=1}^{M_j}\log (1-S_{j_p}(a_{j_p}|x_{j_p}^{(m)}, z_j^{(m)}))\\
    &+\sum_{j=1}^J\frac{1}{M_j}\sum_{m=1}^{M_j}\sum_{i=1}^{n_j}\log f(x_{mis,ij}^{(m)}|W_{ij}, \psi)+\sum_{j=1}^J\frac{1}{M_j}\sum_{m=1}^{M_j}\sum_{i=1}^{n_j}\log f(z_j^{(m)}|\upsilon)
\end{align} 
