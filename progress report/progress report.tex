...
\begin{equation}
    \ell_j(\boldsymbol{\theta})=\sum_{i=1}^{n_j}\delta_{ij}\log(h(t_{ij}|\mathbf{x}_{ij}))+\log\Big (\frac{1}{\sqrt{\pi}}\sum_{\tilde{p}=1}^{N_{\tilde{p}}}\left [\omega_{\tilde{p}}\exp(\sqrt{2}\sigma y_{\tilde{p}})^{d_j}\exp\Big (-\sum_{i=1}^{n_j}H(t_{ij}|\mathbf{x}_{ij})\exp(\sqrt{2}\sigma y_{\tilde{p}})\Big )\right ]\Big )
\end{equation}
...
\begin{align} 
    E_{\mathbf{X}_{j,1,mis}}(\ell(\boldsymbol{\theta}|\boldsymbol{\theta}^{(r)}))&=\sum_{j=1}^J\Bigg [\sum_{i=1}^{n_j}\int_{\boldsymbol{X}_{j,1,mis}}(\delta_{ij}\log h(t_{ij}|\mathbf{x}_{ij}))+ \\ 
    & + \log\Big (\frac{1}{\sqrt{\pi}}\sum_{\tilde{p}=1}^{N_{\tilde{p}}}\Big [\omega_{\tilde{p}}\exp (\sqrt{2}\sigma y_{\tilde{p}})^{d_j}\exp (-\sum_{i=1}^{n_j}H(t_{ij}|\mathbf{x}_{ij})\exp(\sqrt{2}\sigma y_{\tilde{p}}))\Big]\Big ) - \\
    & - \log(A_j(\boldsymbol{\theta})) + \log f(x_{ij,1,mis}|w_{ij},\boldsymbol{\psi}) dx_{ij,1,mis}\Bigg ]
\end{align}
