...
\glsxtrnewsymbol[description={Failure/Event or Censored Time}]{t}{\ensuremath{t}}
...
\glsxtrnewsymbol[description={Covariates design matrix}]{X}{\ensuremath{\mathbf{x}} or \ensuremath{\mathbf{X}}}
...
\end{align}
For each family $j$, the ascertainment $A_j$ is defined to be the probability of the proband $p$ being ascertained by the age $a_{j_p}$ at examination, i.e., $A_j=P(T_{p_j} < a_{p_j})$ where $a_{p_j}$ is proband's age at study entry. Applying the ascertainment correction for the log-likelihood in family $j$: 
\begin{equation}
...
\end{align}
We can firstly take the expectations with respect to $z_j$, to derive a closed-form log-likelihood when the frailty term is integrated. 
We will keep $f(z_j|\upsilon)$ for the frailty distribution in the E-step.
...
\begin{align} 
    ...
    &\times f(\mathbf{x}_{ij,mis}|\mathbf{x}_{ij,obs},\psi^{(r)})f(z_j|\upsilon^{(r)})d\mathbf{x}_{ij,mis}dz_j\\
    &-\sum_{j=1}^J \int_{\mathbf{x}_{mis}} \int_{z_j} \log(1- \exp(z_j H_{j_p}(a_{j_p}|\mathbf{x}_{j_p})))\\
    &\times f(\mathbf{x}_{ij,mis}|\mathbf{x}_{ij,obs},\psi^{(r)})f(z_j|\upsilon^{(r)})d\mathbf{x}_{ij,mis}dz_j
\end{align}
Note that we are able to obtain the integral of the frailty from the above E-step. 
Then we apply the MCEM on the missing data. 
