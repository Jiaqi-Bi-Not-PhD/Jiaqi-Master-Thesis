...
\end{equation}
where $\tilde{\ell}$ is the log-likelihood with ascertainment correction, and $\ell$ is the crude log-likelihood. Define $\mathbf{x}_{p_j}$ the covariate matrix for proband in family $j$. Note we can still apply Laplace transform here, such that
\begin{align}
...
\begin{enumerate} 
    ...
    Therefore, we will use $\mathbf{\tilde{x}}_{ij}$ to simply denote complete covariates. 
    \item In order to approach the joint distribution, we now need to sample the frailty $z_j$ from 
    \begin{align} 
        f(z_j|\mathbf{\tilde{x}}_{ij}, w_{ij}, T_{p_j}<a_{p_j}, \boldsymbol{\theta}^{(r)})\propto \prod_{i=1}^{n_j}f(w_{ij}|\mathbf{\tilde{x}}_{ij}, T_{p_j}<a_{p_j}, \Lambda^{(r)})\times f(z_j|\upsilon^{(r)})
    \end{align}
