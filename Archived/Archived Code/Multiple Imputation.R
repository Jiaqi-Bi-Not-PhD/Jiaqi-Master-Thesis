...
kinship_mat <- with(brca1_noproband, kinship(id = indID, dadid = fatherID, momid = motherID,
                                       sex = rep(2,nrow(brca1_noproband))))
kinship_mat_sparse <- Matrix(kinship_mat, sparse = TRUE)
kinship_mat_sparse <- 2 * kinship_mat_sparse
kinship_mat <- as.matrix(kinship_mat_sparse)

Iden_mat <- diag(nrow = nrow(brca1_noproband)) # Identity matrix n*n
Iden_mat_sparse <- Matrix(Iden_mat, sparse = TRUE)

## Step 2 - empirical estimates
model_test <- relmatLmer(PRS ~ mgeneI + log(timeBC)*BC + (1|indID), data = brca1_noproband, relmat = list(indID = kinship_mat))
summary(model_test)
X <- model.matrix( ~ mgeneI + log(timeBC)*BC, data = brca1_noproband) 

#betas <- coef(model_test)
betas <- c(0.74475, -0.35333, -0.21017, 0.03443, 0.01403)
...
for (m in 1:20) {
  ...
  conditional_expectations <- parSapply(cl, 1:nrow(brca1_noproband), E_cond)
  conditional_expectations <- as.vector(do.call(rbind, conditional_expectations))
  
  conditional_vars <- brca1_noproband$cond_var
  
  ## Step 8
  w2i <- rnorm(n = nrow(brca1_noproband), mean = 0, sd = 1)
  
  ## Step 9
  PRS_star <- conditional_expectations + w2i * conditional_vars
  brca1_prs_imp_fam[[m]] <- brca1_noproband |>
...
coxme_results <- list()
for (i in 1:20) {
  m <- coxme(Surv(timeBC, BC) ~ mgeneI + PRS_I + (1|indID), data = brca1_prs_imp_fam[[i]], varlist = kinship_mat)
  coxme_results[[i]] <- m$coefficients
}
