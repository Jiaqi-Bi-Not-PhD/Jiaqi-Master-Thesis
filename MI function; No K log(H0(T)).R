...
####################################################################################
####################################################################################
MI_FamEvent_noK_H0T <- function(data, M = 5, option = "General", frailty.dist = "gamma") {
  attempts <- 0
  max_attempts <- 3
  success <- FALSE
  ...
  for (i in 1:M) {
    ...
    if (i <= 2) {
      updates_impdata <- penmodel(survival::Surv(time, status) ~ gender + mgene + newx_I, cluster = "famID", gvar = "mgene", 
                                  design = "pop", base.dist = "Weibull", frailty.dist = frailty.dist, 
                                  agemin = min(data_H0$currentage[data_H0$status == 1]), 
                                  data = data_H0,
  ...
  for (i in 1:M) {
    gamma_results[[i]] <- penmodel(survival::Surv(time, status) ~ gender + mgene + newx_I, cluster = "famID", 
                                   ...
                                   frailty.dist = frailty.dist, agemin = min(data_imp[[i]]$currentage[data_imp[[i]]$status == 1]),
...
#test_imp <- MI_FamEvent_noK_H0T(data = data)
